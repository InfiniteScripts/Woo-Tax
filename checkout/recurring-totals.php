						...
						<?php foreach ( $recurring_cart->get_tax_totals() as $recurring_code => $recurring_tax ) : ?>
							...
							<tr class="tax-rate tax-rate-<?php echo esc_attr( sanitize_title( $recurring_code ) ); ?> recurring-total">
								<?php if ( $display_th ) : $display_th = false; ?>
									...
									<td data-title="<?php // echo esc_attr( $recurring_tax->label ); ?>"><?php  //echo '$' . $recurring_cart->get_taxes_total() . ' One time'; ?></td> -->
								<?php else : ?>
									<th></th>
				...
				<?php foreach ( $recurring_carts as $recurring_cart_key => $recurring_cart ) : ?>
					...
					<tr class="tax-total recurring-total">
						<?php if ( $display_th ) : $display_th = false; ?>
							...
							<td data-title="<?php // echo esc_attr( WC()->countries->tax_or_vat() ); ?>"><?php// echo  $recurring_cart->get_taxes_total() . ' Once'; ?></td> -->
						<?php else : ?>
							<th></th>
		...
		<?php foreach ( $recurring_carts as $recurring_cart_key => $recurring_cart ) : ?>
			...
			<tr class="order-total recurring-total">
				<?php if ( $display_th ) : $display_th = false; ?>
					...
					<td data-title="<?php esc_attr_e( 'Recurring Total', 'woocommerce-subscriptions' ); ?>"><?php wcs_cart_totals_subtotal_html( $recurring_cart ); ?></td>
				<?php else : ?>
